...
// FlightAware 個別フライトページから出発地・到着時刻を補完取得
async function fetchFlightDetails(callsign) {
  try {
    ...
    // 出発地
    const ob = html.match(/"origin":\s*\{([^}]{0,400})\}/)?.[1] || '';
    const oIata = ob.match(/"iata":"([^"]+)"/)?.[1] || '';
    const oLoc  = ob.match(/"friendlyLocation":"([^"]+)"/)?.[1] || '';
    const origin = (!oIata || oIata === 'HND' || oIata === 'RJTT') ? null
      : (IATA_JP[oIata] || oLoc.replace(/, Japan$/, '') || oIata);

    // 到着時刻（実績→推定→ゲート の優先順）
    const am = html.match(/"actualArrivalTime":\{"epoch":(\d+)/)
            || html.match(/"estimatedArrivalTime":\{"epoch":(\d+)/)
            || html.match(/"gateArrivalTime":\{"epoch":(\d+)/);
    let arrStr = null;
    if (am) {
      const jst = new Date(parseInt(am[1], 10) * 1000 + 9 * 3600000);
...
export default async function handler(req, res) {
  try {
    ...
    // 出発地・到着時刻が欠けている便を個別ページで補完（最大5件並列）
    const nowMs2 = Date.now();
    const jn = new Date(nowMs2 + 9 * 3600000);
    const [Y2, M2, D2] = [jn.getUTCFullYear(), jn.getUTCMonth(), jn.getUTCDate()];
    await Promise.all(
      merged.filter(f => !f.origin || !f.arrStr).slice(0, 5).map(async f => {
        const det = await fetchFlightDetails(f.flight);
        if (!f.origin  && det.origin) f.origin = det.origin;
        if (!f.arrStr  && det.arrStr) {
          f.arrStr = det.arrStr;
          f.arrMs  = parseJstTime(det.arrStr, Y2, M2, D2, nowMs2) || f.arrMs;
        }
      })
    );
